...
# Keep the installer quiet and present progress ourselves, similar to modern CLI installers.
...
if [[ -d "${INSTALL_ROOT}/.git" ]]; then
  step "Updating RPG"
  git -C "${INSTALL_ROOT}" fetch --depth 1 origin main >/dev/null 2>&1 \
    || fail "Could not fetch the latest RPG version."
  git -C "${INSTALL_ROOT}" reset --hard origin/main >/dev/null 2>&1 \
    || fail "Could not update the RPG installation."
  done_step "RPG updated"
else
  step "Downloading RPG"
  rm -rf "${INSTALL_ROOT}"
  git clone --depth 1 --quiet "${REPO_URL}" "${INSTALL_ROOT}" \
    || fail "Could not download the RPG repository."
...
".venv/bin/python" -m pip install -e ".[dev]" >/dev/null 2>&1 \
  || fail "Could not install RPG components."
...
case ":${PATH}:" in
  ...
  *)
    ...
    printf '  New shells may need PATH updated before `RPG` is available.\n'
    ;;
esac
